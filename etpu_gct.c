...
#include "etpu_uart.h"       /* eTPU function UART API */
...
#if defined(MPC5676R) || defined(MPC5777C)
...
/*******************************************************************************
 * eTPU channel settings - UART 1 (basic)
 ******************************************************************************/
struct uart_instance_t uart_1_instance = 
{
    EM_AB,
    ETPU_UART_1_RX_CHAN,
    ETPU_UART_1_TX_CHAN,
...
struct uart_config_t uart_1_config = 
{
    FS_ETPU_TCR1,
    16,
    ETPU_UART_PARITY_NONE, /* no parity */
    200000, /* 200KHz */
    2,
    80,
    80,
    60, /* rx_fifo_interrupt_threshold */
    10, /* tx_fifo_interrupt_threshold */
    0,  /* RTS not enabled */
    0,  /* RTS not enabled */
    0,  /* RS-485 TXE not enabled */
};

/*******************************************************************************
 * eTPU channel settings - UART 2 (all hw flow control enabled)
 ******************************************************************************/
struct uart_instance_t uart_2_instance = 
{
    EM_AB,
    ETPU_UART_2_RX_CHAN,
    ETPU_UART_2_TX_CHAN,
    ETPU_UART_2_CTS_CHAN,
    ETPU_UART_2_RTS_CHAN,
    ETPU_UART_2_TXE_CHAN,
    FS_ETPU_PRIORITY_MIDDLE,
    0,
    0,
    0,
...
struct uart_config_t uart_2_config = 
{
    FS_ETPU_TCR1,
    8,
    ETPU_UART_PARITY_ODD, /* odd parity */
    100000, /* 100KHz */
    2,
    20,
    20,
    14,  /* rx_fifo_interrupt_threshold */
    2,   /* tx_fifo_interrupt_threshold */
    18,  /* rts_halt_threshold */
    12,  /* rts_resume_threshold */
    20,  /* tx_enable_half_bit_count */
};
...
int32_t my_system_etpu_init(void)
{
  ...
  ...
    ...
    err_code = etpu_uart_init(&uart_1_instance, &uart_1_config);
    if(err_code != FS_ETPU_ERROR_NONE) return(err_code + (ETPU_UART_1_RX_CHAN<<16));

    err_code = etpu_uart_init(&uart_2_instance, &uart_2_config);
    if(err_code != FS_ETPU_ERROR_NONE) return(err_code + (ETPU_UART_2_RX_CHAN<<16));
...
{
#if 0 /* do these steps later, as needed, in this test app */
